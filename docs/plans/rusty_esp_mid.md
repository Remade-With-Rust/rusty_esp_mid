...
| `Verifier::verify`, `verify_roster_update` | upstream `kms-verifier` · `verifier.rs` | signed assertions and roster updates from the device | **rejects** (an explicit `normalize_s().is_some()` check before the nonce is consumed); pinned by the oracle test `kms_assertion_verifies_with_kms_verifier_and_replay_is_refused`, which also proves a refused twin does not burn the nonce |
